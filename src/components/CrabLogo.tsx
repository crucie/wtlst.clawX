...
import {
  ...
  RENDER_ORDER, MORPH_STEP, SPAWN_STEP,
  mkCircle, samplePath, interpPath,
  easeOutBack, easeOutCubic, easeOutQuint, easeOutExpo,
  clamp, norm,
  type Pt,
} from "@/lib/crab-paths";
...
const T_SETTLE        = 0.3;
const ANIM_END        = ALL_MORPHED + T_SETTLE;
...
export default function CrabLogo({ className, style }: CrabLogoProps) {
  ...
  useEffect(() => {
    ...
    // ── Sample both circle and final shapes (hidden off-screen) ──────────
    ...
    const groupEl = svg.querySelector<SVGGElement>(".crab-g");
    ...
    function tick(now: number): void {
      ...
      // ── Animation complete → lock final state, stop loop ───────────────
      if (t >= ANIM_END) {
        pathEls.forEach((el, i) => {
          if (!el) return;
          el.setAttribute("d", CRAB_SHAPES[i].d);
          el.style.opacity = "1";
          el.style.filter = "";
        });
        if (groupEl) groupEl.removeAttribute("transform");
        return; // stop RAF
      }

      // ── Group settle (slight scale snap after morph) ───────────────────
      if (groupEl) {
        let sc = 1;
        if (t > ALL_MORPHED && t < ANIM_END) {
          const st = norm(t, ALL_MORPHED, ANIM_END);
